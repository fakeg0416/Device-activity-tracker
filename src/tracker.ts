...
export class WhatsAppTracker {
     ...
     * All device variants (number:1@s.whatsapp.net, @lid, etc.) belong
     * to the same contact, so we merge them into one entry.
     */
    ...
    public async startTracking() {
                ...
                for (const [jid, presenceData] of Object.entries(update.presences)) {
                    if (presenceData && presenceData.lastKnownPresence) {
                        ...
                        if (!this.deviceMetrics.has(normJid)) {
                            ...
                            // if WhatsApp confirms the user is online, they're online.
        ...
        this.intervalIds.push(presenceStaleInterval);
    ...
    private addMeasurementForDevice(jid: string, rtt: number) {
        ...
        if (rtt <= 3500) {
            ...
            // 2. Add to device's history for calibration (last 2000), filtering outliers > 3500ms
    ...
    public stopTracking() {
        ...
        // Clear all intervals (presence subscription, stale check)
